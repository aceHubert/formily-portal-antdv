...
export interface PageLayoutProps {
   ...
   * container width
   */
  containerWidth?: 'fullwidth' | string | number
  /**
...
export const PageLayout = defineComponent<PageLayoutProps>({
  ...
  setup(customProps: PageLayoutProps, { attrs, slots }) {
    const prefixCls = usePrefixCls('portal-page', attrs.prefixCls as string)
    ...
    return () => {
      ...
      const style = {}
      if (
        props.value.containerWidth &&
        props.value.containerWidth !== 'fullwidth'
      ) {
        style['minWidth'] = props.value.containerWidth
      }
