...
import { parseStyleUnit, warn } from '../__builtins__'
...
export interface IUsePropsCalcPageLayout {
  (props: IProps): {
    props: Ref<
      Omit<IProps, 'containerWidth' | 'scopedDataRequest'> & {
        containerWidth: Extract<IProps['containerWidth'], string>
...
export const usePropsCalcPageLayout: IUsePropsCalcPageLayout = (props) => {
  const layoutProps = computed(() => {
    return {
      ...props,
      containerWidth: parseStyleUnit(props.containerWidth),
